...
Remote Codex safety (#736): `spawnNew` refuses managed SSH Codex accounts (including custom
Codex harnesses and known agent-less login terminals) until remote validation and per-account
hooks are wired. System Codex retains the host environment through `remoteAccountScopeEnvArgs`,
even before remoteHome discovery during early attach. Never guess HOME/CODEX_HOME. Desktop and
Server share this core gate; remote account management remains unavailable.
